...
//PWM
void NextPWM(uint8_t motor)
{
	//Actualizar DutyCycle
	//-----------------------------------------------------------------------------------------------
	//if (DutyCycle != DutyCycle0)

		Chip_PWM_SetMatch(LPC_PWM1, motor, DutyCycle[motor]);
		Chip_PWM_Reset(LPC_PWM1);
		//DutyCycle0 = DutyCycle;
	...
	{
	case 0:
		Chip_GPIO_WritePortBit(LPC_GPIO, PORT_Qa[motor], PIN_Qa[2], 0);	//Apago Q4
	...
	if (StepID[motor] > 4)	//Si StepID es mayor a 4 reseteo variable StepID
	{
		StepID = 0;
		Count++;
	} else
...
void PWM1_IRQHandler(void)
{
	...
	if (Chip_PWM_MatchPending(LPC_PWM1, 0))	//Reviso interrupción pendiente canal PWM 0
	{
		...
		Chip_GPIO_WritePortBit(LPC_GPIO, PORT_Q1b[Cycle], PIN_Q1b[Cycle], 0);	//Encender (invertido por el tbj emisor común)
		Chip_GPIO_WritePortBit(LPC_GPIO, PORT_Q2b[Cycle], PIN_Q2b[Cycle], 0);	//Encender (invertido por el tbj emisor común)
		Chip_GPIO_WritePortBit(LPC_GPIO, PORT_Q3b[Cycle], PIN_Q3b[Cycle], 0);	//Encender (invertido por el tbj emisor común)
		Chip_GPIO_WritePortBit(LPC_GPIO, PORT_Q4b[Cycle], PIN_Q4b[Cycle], 0);	//Encender (invertido por el tbj emisor común)
	}
	//Interrupción Canal 3 -> DUTYCICLE
	//-----------------------------------------------------------------------------------------------
	if (Chip_PWM_MatchPending(LPC_PWM1, 3)) //Reviso interrupción pendiente canal PWM 5
	{
		Chip_PWM_ClearMatch(LPC_PWM1, 3);	//Limpio interrupción canal PWM 5

		//PWM sobre transistores NMOS
		Chip_GPIO_WritePortBit(LPC_GPIO, PORT_Q1b[Cycle], PIN_Q1b[Cycle], 1);	//Apagar (invertido por el tbj emisor común)
	}
	//Interrupción Canal 4 -> DUTYCICLE
	//-----------------------------------------------------------------------------------------------
	if (Chip_PWM_MatchPending(LPC_PWM1, 4)) //Reviso interrupción pendiente canal PWM 5
	{
		Chip_PWM_ClearMatch(LPC_PWM1, 4);	//Limpio interrupción canal PWM 5

		//PWM sobre transistores NMOS
		Chip_GPIO_WritePortBit(LPC_GPIO, PORT_Q2b[Cycle], PIN_Q2b[Cycle], 1);	//Apagar (invertido por el tbj emisor común)
	}
	...
	if (Chip_PWM_MatchPending(LPC_PWM1, 5)) //Reviso interrupción pendiente canal PWM 5
	{
		...
		//PWM sobre transistores NMOS
		Chip_GPIO_WritePortBit(LPC_GPIO, PORT_Q3b[Cycle], PIN_Q3b[Cycle], 1);	//Apagar (invertido por el tbj emisor común)
	}
	//Interrupción Canal 5 -> DUTYCICLE
	//-----------------------------------------------------------------------------------------------
	if (Chip_PWM_MatchPending(LPC_PWM1, 6)) //Reviso interrupción pendiente canal PWM 5
	{
		...
		//PWM sobre transistores NMOS
		Chip_GPIO_WritePortBit(LPC_GPIO, PORT_Q4b[Cycle], PIN_Q4b[Cycle], 1);	//Apagar (invertido por el tbj emisor común)
	}
}
